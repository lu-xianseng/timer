...
from PyQt5.QtWidgets import QApplication, QWidget, QStyle, \
    ...
    QCheckBox, QMessageBox, QMainWindow, QMenuBar, QMenu, QAction, \
    QRadioButton, QTableWidget, QTableWidgetItem, QStackedWidget, QHeaderView
from PyQt5.QtGui import QFont, QPalette, QColor, QIcon
from PyQt5.QtCore import Qt, QTimer
from database import DatabaseManager
from styles import (set_label_styles, set_line_edit_styles,
                    set_button_styles, set_checkbox_styles,
                    set_radio_styles
                    )
...
class LorienTimer(QMainWindow):
    ...
    def check_and_delete_task(self, task_name):
        try:
            result = subprocess.run(f'schtasks /query /tn "{task_name}"', shell=True, stdout=subprocess.PIPE,
        ...
        try:
            ...
            if 0 <= hour <= 23 and 0 <= minute <= 59:
                ...
                if self.selected_days:
                    ...
                    print(f"定时器启动，shutdown_time: {self.shutdown_time}")
    ...
    def check_existing_tasks(self):
        ...
        if tasks:
            ...
            for task in tasks:
                ...
                target_time = time.mktime((target_time.tm_year, target_time.tm_mon, target_time.tm_mday, shutdown_hour, shutdown_minute, 0, 0, 0, 0))
            ...
            if next_time:
                ...
                print(f"检查到现有任务，shutdown_time: {self.shutdown_time}")
                self.timer.start(1000)
                self.update_countdown()
        else:
            self.countdown_label.setText("未设置定时任务")
    ...
    def update_countdown(self):
        if self.shutdown_time:
            ...
            print(f"当前时间: {time.strftime('%Y-%m-%d %H:%M:%S', now)}, 设置时间: {hour}:{minute}, 循环日期: {days}")
            next_time = None
            for day in days:
                target_time = time.mktime(now)
                current_weekday = now.tm_wday + 1
                ...
                target_time += days_diff * 24 * 60 * 60
                target_time = time.localtime(target_time)
                target_time = time.mktime((target_time.tm_year, target_time.tm_mon, target_time.tm_mday, hour, minute, 0, 0, 0, 0))
                if target_time < time.mktime(now):
                    target_time += 7 * 24 * 60 * 60
                if next_time is None or target_time < next_time:
                    next_time = target_time
            print(f"下一次任务时间: {next_time if next_time else '未找到'}")
            if next_time:
